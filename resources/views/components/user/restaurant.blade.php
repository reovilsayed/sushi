            ...
            @foreach ($restaurants as $restaurant)
                @if ($restaurant->status == '1')
                    <div class="col-lg-4" data-aos="fade-up" data-aos-delay="100">
                        <a href="{{ $restaurant->status == 1 ? route('restaurant.menu', $restaurant->slug) : 'javascript:void(0)' }}"
                            onclick="{{ $restaurant->status != 0 ? 'window.history.back()' : "alert('The restaurant is closed.!')" }}">
                            <div class="member">
                                <img src="{{ $restaurant->image ? Storage::url($restaurant->image) : asset('niko/image/restaurant.jpg') }}"
                                    class="img-fluid" alt="">
                                <div class="member-info">
                                    <div class="member-info-content">
                                        <h4>{{ session()->get('locale') =='ar' ? $restaurant->name_ae : $restaurant->name }}</h4>
